...
const handler = async (req) => {
  ...
  prompt =
    ...
    `2) FACE RETOUCH: ${retouch}At EVERY retouch level the person must remain instantly recognisable — no slimming, no enlarging eyes, no changing the nose, jaw or lips into different features. Subtle adjustments ARE allowed and encouraged for a natural result: re-pose the head (turn/tilt) to fit the requested pose, match the scene's lighting and colour on the face, blend the head seamlessly with the body at the correct scale, apply gentle professional polish, and the requested facial EXPRESSION. ` +
    `3) CLOTHING: ` +
    (outfitOrd
